...
begin
  ...
  totals as (
    select
      ...
      coalesce(sum(da.area_m2) filter (
        where da.local_date between v_reference_date - 6 and v_reference_date
      ), 0)::double precision as current_period_area_m2,
      coalesce(sum(da.area_m2) filter (
        where da.local_date between v_reference_date - 13 and v_reference_date - 7
      ), 0)::double precision as previous_period_area_m2
  ...
  ranked as (
    select
      ...
      p.territory_color,
      t.total_area_m2,
      (t.current_period_area_m2 - t.previous_period_area_m2)::double precision as delta_area_m2,
