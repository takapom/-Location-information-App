...
serve(async (req) => {
  ...
  const { data, error } = await supabase.rpc("sync_live_territory", {
    p_daily_activity_id: dailyActivityId
  });

  if (error) {
    return new Response(JSON.stringify({ error: "sync_failed", message: error.message }), { status: 400, headers: jsonHeaders });
  }
