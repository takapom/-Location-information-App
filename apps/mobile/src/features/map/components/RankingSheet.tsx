...
export function RankingSheet({ rankings, onFriends, onClose }: RankingSheetProps) {
  const topThree = rankings.slice(0, 3);

  return (
    <BottomSheet height="72%" onClose={onClose}>
      <View style={styles.sheetHeader}>
        <Text style={styles.rankingTitle}>🏆 フレンドランキング</Text>
        <TouchableOpacity accessibilityLabel="ランキングを閉じる" accessibilityRole="button" onPress={onClose} style={styles.closeCircle} testID="ranking-close-button">
          <Text style={styles.closeText}>×</Text>
        </TouchableOpacity>
      </View>
      <ScrollView style={styles.rankingScroll} contentContainerStyle={styles.rankingScrollContent} showsVerticalScrollIndicator testID="ranking-scroll-view">
