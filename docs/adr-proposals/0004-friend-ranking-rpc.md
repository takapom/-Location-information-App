# ADR Proposal 0004: 友達ランキングを承認済み友達スコープRPCへ閉じる
...
S04のランキング表示はmockでは複数ユーザーを表示できるが、Supabase実装では現在ユーザーのみを返している。友達ランキングは友達関係を認可境界にするため、mobileから直接 `profiles` や他ユーザーの活動履歴を組み合わせると、非友達の面積やプロフィールを露出する危険がある。
