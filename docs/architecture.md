...
| Entity | 概要 |
|---|---|
| Profile | `auth.users`に紐づくアプリ用プロフィール。友達コード、表示名、アバター、陣地色、位置共有設定を持つ |
...
9. 表示用にポリゴンを簡略化する。
10. live同期では`territories(state='live')`へupsertし、「今日を確定」操作時の`finalize_daily_activity`で`territories(state='final')`へ保存する。
