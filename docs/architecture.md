...
| UXは即時反応が重要 | STOP前にも半透明プレビューを出す必要がある | クライアントで軽量プレビュー、サーバーで確定計算を行う |
...
Supabase
  ...
  - Edge Functions
        ...
        +--> Edge Functions
               - activity completion
               - polygon generation orchestration
...
| Server Logic | Supabase Edge Functions | STOP時の確定処理、GPS検証、外部API連携を集約する |
...
| Supabase Realtime | 友達現在地、オンライン状態、トラッキング中表示など揮発状態のみ |
...
| Activity | 1回の移動セッション。開始/終了、距離、面積、状態を持つ |
| LocationPoint | Activityに紐づくGPS点。自分の活動履歴として保存する |
...
create index location_points_activity_recorded_at_idx
  on public.location_points (activity_id, recorded_at);
...
| トラッキング開始 | `activities` insert RPC or direct insert | active Activity作成 |
| GPS点送信 | `location_points` insert RPC or batched insert | 5秒ごと/10mごとの点保存 |
| トラッキング完了 | Edge Function `complete-activity` | GPS検証、距離計算、ポリゴン確定、Activity完了 |
| 履歴一覧 | `activities` select + RLS | S07 |
| 活動詳細 | `activities`/`territories` select + RLS | S08 |
...
```text
1. mobile: ユーザーがSTART
2. mobile: Supabase Auth sessionからuser_idを確定
3. mobile/db: activitiesにactiveレコードを作成
4. mobile: Realtime Presenceにis_active=trueと最新位置を送る
5. mobile: 5秒ごと or 10m移動ごとにlocation_pointsへGPS点を保存
6. mobile: ローカルで簡易バッファポリゴンを生成し、半透明プレビュー表示
7. mobile: ユーザーがSTOP
8. edge function: complete-activityが保存済みGPS点を検証・確定計算
9. db: territories保存、activitiesをcompleted化
10. mobile: Realtime Presenceをis_active=falseへ更新
11. mobile: S06完了ボトムシートを表示
```

### 6.2 確定アルゴリズム

確定処理は`supabase/functions/complete-activity`から呼び出し、共通ロジックは`packages/geo/territory-builder.ts`に置く。PostGISで安定して処理できる部分はSQL/RPCへ寄せる。

1. Activityの所有者が`auth.uid()`と一致することを確認する。
2. GPS点を時系列で取得する。
...
| active状態 | トラッキング中なら`isActive = true` |
...
| `tracking` | GPS permission、開始/停止、送信間隔制御、プレビュー、Presence更新 |
| `activities` | 履歴、詳細、完了結果 |
...
| 自分の履歴 | Activity用GPS点は自分の履歴としてPostgresに保存する |
...
- PostGIS schema
- RLS
- Activity/LocationPoint/Territory
- Edge Function `complete-activity`
- サーバー確定ポリゴン生成
- クライアントプレビュー
...
- Activity/territory/contextから提案生成
- 提案履歴または一時表示
...
3. Supabase migrationでPostGIS、profiles、activities、location_points、territoriesを作る。
4. RLSを有効化し、本人データだけ読める/書ける状態にする。
5. `packages/shared`にschemaと型を作る。
6. `packages/geo`にGPSフィルタ、距離、プレビュー、territory builderのテストを作る。
7. `complete-activity` Edge Functionを作る。
8. `apps/mobile`にMapbox表示、トラッキング開始/停止、GPS保存を作る。
9. S06/S07/S08で確定結果と履歴を表示する。
...
単純な本人CRUDは`supabase-js`からRLS付きで直接アクセスする。Activity完了、友達招待、ランキング集計など整合性・認可・複数テーブル更新が必要な処理はEdge FunctionまたはRPCに寄せる。
